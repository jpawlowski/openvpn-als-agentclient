...
public class AgentLauncher extends Applet implements ApplicationLauncherEvents {
    ...
    public void launch() {
        try {
            ...
            httpsPort = Integer.parseInt(this.getParameter("httpsPort", "0"));
            protocol = this.getParameter("protocol", null);
            hostaddr = this.getParameter("hostaddr", null);
            ...
            Thread thread = new Thread() {
                ...
                public void run() {
                    ...
                    try {
                        ...
                        int port = codebase.getPort() == -1 ? (codebase.getProtocol()!=null && codebase.getProtocol().equalsIgnoreCase("http") ? 80 : 443) : codebase.getPort();
                        if (httpsPort != 0 && port != httpsPort)
                        	port = httpsPort;
                        if (protocol == null)
                        	protocol = codebase.getProtocol();
                        if (hostaddr == null)
                        	hostaddr = codebase.getHost().equals("") ? "localhost" : codebase.getHost();

                        AbstractApplicationLauncher launcher = new AgentLauncherApplicationLauncher(new File(Utils.getHomeDirectory(), cacheDirectory),  protocol, null, hostaddr //$NON-NLS-1$
                                        , //$NON-NLS-1$ //$NON-NLS-2$
                                        	port, params, AgentLauncher.this);
